...
export const menuService = {
  ...
  getItem:      (id)          => api.get(`/menu/${id}`).then(r => r.data),
  ...
  createItemForm: (formData) => api.post('/menu', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  }),
  updateItemForm: (id, formData) => api.post(`/menu/${id}`, formData, {
    // Laravel needs POST + _method=PUT for FormData
    headers: { 'Content-Type': 'multipart/form-data' },
  }),
...
export const orderService = {
  ...
  getQueue:     (params = {}) => api.get('/orders-queue', { params }).then(r => r.data),
...
export const inventoryService = {
  ...
  getLowStockAlerts:()            => api.get('/inventory/low-stock').then(r => r.data),
  restockItem:      (id, quantity, reason = '') =>
    api.post(`/inventory/${id}/restock`, { quantity, reason }).then(r => r.data),
  bulkRestock:      (items, reason = '') =>
    api.post('/inventory/bulk-restock', { items, reason }).then(r => r.data),
  adjustStock:      (id, quantity, type, reason = '') =>
    api.post(`/inventory/${id}/adjust`, { quantity, type, reason }).then(r => r.data),
  getLogs:          (params = {}) => api.get('/inventory/logs', { params }).then(r => r.data),
};
...
export const reportService = {
  getSalesSummary:    (params = {}) => api.get('/reports/sales-summary', { params }).then(r => r.data),
  getBestSellers:     (params = {}) => api.get('/reports/best-sellers', { params }).then(r => r.data),
  getTopItems:        (params = {}) => api.get('/reports/best-sellers', { params }).then(r => r.data),
  getSalesByCategory: (params = {}) => api.get('/reports/sales-by-category', { params }).then(r => r.data),
  getOrderTrends:     (params = {}) => api.get('/reports/order-trends', { params }).then(r => r.data),
  exportCsv: (params = {}) => api.get('/reports/export-csv', { params, responseType: 'blob' }).then(r => {
